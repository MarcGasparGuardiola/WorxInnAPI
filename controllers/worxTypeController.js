...
const worxTypeService = require('../services/worxTypesService');
...
module.exports = {
  ...
  create: async (req, res) => {
    ...
    try {
      ...
      const resFromService = await worxTypeService.create(space);
  ...
  update: async (req, res) => {
    const response = { status: c.status.serverError, msg: 'Internal server error' };
    try {
      const resFromService = await worxTypeService.update(req.params.id, req.body);
  ...
  delete: async (req, res) => {
    ...
    try {
      ...
      const resFromService = await worxTypeService.delete(spaceId);
      if (resFromService.status) {
        if (resFromService.result > 0) {
          response.status = c.status.ok;
  ...
  selectById: async (req, res) => {
    ...
    try {
      const spaceId = req.params.id;
      const resFromService = await worxTypeService.selectById(spaceId);
      if (resFromService.status) {
  ...
  selectAll: async (req, res) => {
    ...
    try {
      ...
      const resFromService = await worxTypeService.selectAll(queryParams, pagination);
      if (resFromService.status) {
        // response.status = c.status.ok;
        // response.body = resFromService.result;
        if (resFromService.result) {
