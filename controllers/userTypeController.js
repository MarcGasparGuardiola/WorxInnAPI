...
const userTypeService = require('../services/userTypesService');
...
module.exports = {
  ...
  create: async (req, res) => {
    ...
    try {
      ...
      const resFromService = await userTypeService.create(space);
  ...
  update: async (req, res) => {
    const response = { status: c.status.serverError, msg: 'Internal server error' };
    try {
      const resFromService = await userTypeService.update(req.params.id, req.body);
  ...
  delete: async (req, res) => {
    ...
    try {
      ...
      const resFromService = await userTypeService.delete(spaceId);
      if (resFromService.status) {
        if (resFromService.result > 0) {
          response.status = c.status.ok;
  ...
  selectById: async (req, res) => {
    ...
    try {
      const spaceId = req.params.id;
      const resFromService = await userTypeService.selectById(spaceId);
      if (resFromService.status) {
  ...
  selectAll: async (req, res) => {
    ...
    try {
      ...
      const resFromService = await userTypeService.selectAll(queryParams, pagination);
      if (resFromService.status) {
        // response.status = c.status.ok;
        // response.body = resFromService.result;
        if (resFromService.result) {
