...
const spaceService = require('../services/spacePhotoService');
...
module.exports = {
  ...
  create: async (req, res) => {
    ...
    try {
      ...
      const resFromService = await spaceService.create(space);
  ...
  update: async (req, res) => {
    const response = { status: c.status.serverError, msg: 'Internal server error' };
    try {
      const resFromService = await spaceService.update(req.params.id, req.body);
  ...
  delete: async (req, res) => {
    ...
    try {
      ...
      const resFromService = await spaceService.delete(spaceId);
      if (resFromService.status) {
        if (resFromService.result > 0) {
          response.status = c.status.ok;
  ...
  selectById: async (req, res) => {
    ...
    try {
      const spaceId = req.params.id;
      const resFromService = await spaceService.selectById(spaceId);
      if (resFromService.status) {
