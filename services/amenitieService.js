const { Amenitie } = require('../sequelize')

module.exports.selectById = async (spaceId) => {
  const response = { status: false };
  try {
    const resFromRepo = await Amenitie.findOne({
      where: {
        id: spaceId
      }
    });
...
module.exports.selectAll = async (queryParams, pagination) => {
  const response = { status: false };
  try {
    const Spaces = await Amenitie.findAll();
    if (Spaces.length > 0) {
...
module.exports.create = async (SpaceFromController) => {
  ...
  try {
    //TODO: Hash passwordA
    const SpaceCreated = await Amenitie.create(SpaceFromController);
...
module.exports.update = async (id, data) => {
  ...
  try {
    console.log(id)
    console.log(data)
    const resFromRepo = await Amenitie.update(data , {
      where: {
        id: id
      }
    });
...
module.exports.delete = async (SpaceId) => {
  const response = { status: false };
  try {
    const resFromRepo = await Amenitie.destroy({
      where: {
        id: SpaceId
      }
    });
