...
module.exports.selectById = async (spaceId) => {
  ...
  try {
    const resFromRepo = await Review.findOne({
      where: {
        id: spaceId
      }
    });
...
module.exports.selectAll = async (queryParams, pagination) => {
  const response = { status: false };
  try {
    const Spaces = await Review.findAll();
    if (Spaces.length > 0) {
...
module.exports.create = async (SpaceFromController) => {
  ...
  try {
    //TODO: Hash passwordA
    const SpaceCreated = await Review.create(SpaceFromController);
...
module.exports.update = async (id, data) => {
  ...
  try {
    console.log(id)
    console.log(data)
    const resFromRepo = await Review.update(data , {
      where: {
        id: id
      }
    });
...
module.exports.delete = async (SpaceId) => {
  const response = { status: false };
  try {
    const resFromRepo = await Review.destroy({
      where: {
        id: SpaceId
      }
    });
